...
// struct for holding 3D PB locations
struct RectWithLayer{
  vtr::Rect<size_t> coordinates;
  size_t layer;
};
...
class FabricTile {
 public: /* Accessors */
  ...
  FabricTileId find_tile(const PointWithLayer& coord) const;
   ...
   * id */
  PointWithLayer unique_tile_coordinate(const FabricTileId& tile_id) const;
  ...
  PointWithLayer find_cb_coordinate_in_unique_tile(
   ...
   * coordinate. Error out on any exception. */
  PointWithLayer find_pb_coordinate_in_unique_tile(
   ...
   * coordinate. Error out on any exception. */
  PointWithLayer find_sb_coordinate_in_unique_tile(
   ...
   */
  bool pb_in_tile(const FabricTileId& tile_id, const PointWithLayer& coord,
                  const bool& use_gsb_coord = false) const;
 ...
 public: /* Mutators */
  FabricTileId create_tile(const PointWithLayer& coord);
  ...
  void init(const PointWithLayer& max_coord);
 ...
 private: /* Internal builders */
  void invalidate_tile_in_lookup(const PointWithLayer& coord);
  void invalidate_pb_in_lookup(const PointWithLayer& coord);
  void invalidate_cbx_in_lookup(const PointWithLayer& coord);
  void invalidate_cby_in_lookup(const PointWithLayer& coord);
  void invalidate_sb_in_lookup(const PointWithLayer& coord);
 ...
 private: /* Internal Data */
  ...
  vtr::vector<FabricTileId, PointWithLayer> coords_;
   ...
   * resolved.
   */
  vtr::vector<FabricTileId, std::vector<RectWithLayer>> pb_coords_;
  vtr::vector<FabricTileId, std::vector<PointWithLayer>> pb_gsb_coords_;
  vtr::vector<FabricTileId, std::vector<PointWithLayer>> cbx_coords_;
  vtr::vector<FabricTileId, std::vector<PointWithLayer>> cby_coords_;
  vtr::vector<FabricTileId, std::vector<PointWithLayer>> sb_coords_;
  /* A few fast lookup to spot tile by coordinate of programmable blocks,
   * connection blocks and switch blocks using [layer][x][y]*/
  std::vector<std::vector<std::vector<FabricTileId>>> pb_coord2id_lookup_;
  std::vector<std::vector<std::vector<FabricTileId>>> cbx_coord2id_lookup_;
  std::vector<std::vector<std::vector<FabricTileId>>> cby_coord2id_lookup_;
  std::vector<std::vector<std::vector<FabricTileId>>> sb_coord2id_lookup_;
  /* A fast lookup to spot tile by coordinate */
  std::vector<std::vector<std::vector<FabricTileId>>> tile_coord2id_lookup_;
  std::vector<std::vector<std::vector<FabricTileId>>>
    tile_coord2unique_tile_ids_; /* Use [layer][x][y] to get the id of the unique tile
